...
async function hasContratoBaseFirmado(correoPersonal) {
...
async function resolveContratoPersonaContext(proceso) {
  ...
  const nombre = toNullableTrimmedString(solicitud?.nombre) || toNullableTrimmedString(preregistro?.nombre) || toNullableTrimmedString(proceso?.nombre_persona) || "";
  const apellidos = toNullableTrimmedString(solicitud?.apellidos) || toNullableTrimmedString(preregistro?.apellidos) || "";
  const nombreCompleto = `${nombre} ${apellidos}`.trim() || toNullableTrimmedString(proceso?.nombre_persona) || "";
  const tipoDocumentoCodigo = toNullableTrimmedString(solicitud?.tipo_documento_codigo) || toNullableTrimmedString(preregistro?.tipo_documento_codigo) || "";
  const tipoDocumentoTitulo = toNullableTrimmedString(solicitud?.tipo_documento_titulo) || toNullableTrimmedString(preregistro?.tipo_documento_titulo) || "";
...
async function findAnexoDuplicate(payload) {
  const r = await pool.query(
    ...
    WHERE estado <> 'cancelado'
      AND COALESCE(numero_documento, '') = COALESCE($1, '')
      AND COALESCE(LOWER(correo_personal), '') = COALESCE(LOWER($2), '')
      AND tipo_asignacion = $3
      AND COALESCE(cliente_id, 0) = COALESCE($4, 0)
      AND COALESCE(moneda, 'COP') = COALESCE($5, 'COP')
      AND valor_tarifa = $6
      AND fecha_inicio = $7
      AND fecha_fin = $8
    LIMIT 1
...
async function ensureAutomaticAnexoFromContext({ proceso, personaContext, createdBy = null }) {
  ...
  if (valorTarifa === null || !fechas.fecha_inicio || !fechas.fecha_fin) return null;
...
async function buildContratoTemplatePayload({ docDefinition, personaContext, proceso }) {
  ...
  if (docDefinition?.doc_key === "anexo_tecnico") {
    let items = await listAnexoTecnicoItems({
      solicitudId: proceso?.solicitud_id || null,
      preregistroId: proceso?.preregistro_id || null,
    ...
    if (!items.length) {
      try {
        ...
        items = await listAnexoTecnicoItems({
          solicitudId: proceso?.solicitud_id || null,
          preregistroId: proceso?.preregistro_id || null,
          numeroDocumento: personaContext?.numeroDocumento || null,
          correoPersonal: personaContext?.correoPersonal || null
        ...
        console.warn("No se pudo crear item automatico de anexo tecnico:", autoErr?.message || autoErr);
      }
    }

    const rows = (items || []).map(buildAnexoItemForTemplateRow);
    if (!rows.length) {
      const tipo = inferAnexoTipoFromContext(personaContext);
      const tarifa = inferAnexoTarifaFromContext(personaContext, tipo);
      const fechas = inferAnexoDatesFromContext(personaContext, tipo);
      rows.push(
        buildAnexoItemForTemplateRow({
          tipo_asignacion: tipo,
          cliente_nombre: personaContext?.clienteNombre || "",
          valor_tarifa: tarifa || 0,
          moneda: personaContext?.moneda || "COP",
          fecha_inicio: fechas.fecha_inicio,
          fecha_fin: fechas.fecha_fin
        })
      );
      ...
      pool.query(`
        SELECT
          ...
          EXISTS (
            ...
            WHERE LOWER(tf.correo_personal) = LOWER(sc.correo_personal)
              AND tf.estado = 'completado'
      ...
      pool.query(`
        SELECT
          ...
          EXISTS (
            SELECT 1
            FROM tokens_firma_contrato tf
            WHERE LOWER(tf.correo_personal) = LOWER(pp.correo_personal)
              AND tf.estado = 'completado'
          ) AS tiene_contrato_base,
          pp.created_at
        FROM preregistro_personas pp
...
app.post("/admin/firma-contratos/generar", requireAccess({ roles: ["Administrador", TALENTO_HUMANO_ROL] }), async (req, res) => {
  ...
  try {
    ...
    const hasBaseContract = await hasContratoBaseFirmado(correo_personal);
    ...
    const insert = await pool.query(
      ...
      [token, solId, preId, nombre_persona, correo_personal, JSON.stringify(docsPlan), req.user.id, expiresAt]
    );
    ...
    try {
      ...
      await ensureAutomaticAnexoFromContext({
        proceso: procesoContext,
        personaContext,
        createdBy: req.user.id
      });
    } catch (anexoErr) {
      console.warn("No se pudo asegurar item automatico de anexo al generar token:", anexoErr?.message || anexoErr);
    }
...
app.get("/admin/firma-contratos/anexo-items", requireAccess({ roles: ["Administrador", TALENTO_HUMANO_ROL] }), async (req, res) => {
  try {
    ...
    if ((solicitudId || preregistroId) && !numeroDocumento && !correoPersonal) {
      ...
      numeroDocumento = toNullableTrimmedString(personaContext?.numeroDocumento);
      correoPersonal = toNullableTrimmedString(personaContext?.correoPersonal);
    }
...
app.get("/admin/usuarios/:id/licencias-historial", requireAccess({ roles: ["Administrador"] }), async (req, res) => {
  const { id } = req.params;
  try {
    const userRes = await pool.query(
  ...
  try {
    ...
    if (docDefinition.doc_key === "anexo_tecnico") {
      try {
        ...
        console.warn("No se pudo asegurar anexo tecnico antes de firmar:", anexoErr?.message || anexoErr);
      }
    }
...
app.get("/tarifa-consultor", requireAccess({ roles: ["Administrador", "Coordinador"] }), async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `
      WITH 
        c_consultor AS (SELECT id FROM usuarios WHERE public_id = $1),
      ...
      SELECT obtener_tarifa_consultor(
    ...
    // Si todo es null porque UUIDs son malos la función va a arrojar null
    res.json(withPublicId(result.rows[0]));
  } catch (err) {
    if (err?.code === "PUBLIC_ID_NOT_FOUND") {
...
app.put("/tarifas/:id", requireAccess({ roles: ["Administrador", "Coordinador"] }), async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `
      WITH 
        c_tarifa AS (SELECT id FROM tarifa_consultor WHERE public_id = $6),
        c_cliente AS (SELECT id, titulo FROM clientes WHERE public_id = $1),
        c_consultor AS (SELECT id, nombre_usuario, moneda_cobro FROM usuarios WHERE public_id = $2),
        c_modulo AS (SELECT id, titulo FROM modulo WHERE public_id = $3),
        c_tipo_asignacion AS (SELECT id, titulo FROM tipo_asignacion WHERE public_id = $4),
        
        upd AS (
          UPDATE tarifa_consultor
          SET id_cliente = (SELECT id FROM c_cliente),
              consultor_id = (SELECT id FROM c_consultor),
              modulo_id = (SELECT id FROM c_modulo),
          ...
          RETURNING id, id_cliente, consultor_id, modulo_id, id_tipo_asignacion, valor_tarifa, activo, public_id
        )
      SELECT
        upd.public_id AS id,
        $1 AS cliente_id,
        $2 AS consultor_id,
        $3 AS modulo_id,
        $4 AS tipo_asignacion_id,
        upd.valor_tarifa AS valor,
        upd.activo,
        (SELECT titulo FROM c_cliente) AS nombre_cliente,
        (SELECT nombre_usuario FROM c_consultor) AS nombre_consultor,
        (SELECT titulo FROM c_modulo) AS nombre_modulo,
        (SELECT titulo FROM c_tipo_asignacion) AS tipo_asignacion,
        (SELECT moneda_cobro FROM c_consultor) AS moneda
      FROM upd
      `,
      [
        cliente_id,
        consultor_id,
        modulo_id || null,
        tipo_asignacion_id, // NOT NULL EN DB AL PARECER SEGUN REQ
        valor,
        id
      ]
    );
    ...
    if (result.rowCount === 0) {
      // Analizar fallo
      const checkT = await pool.query("SELECT id FROM tarifa_consultor WHERE public_id = $1", [id]);
      if (checkT.rowCount === 0) return res.status(404).json({ error: "Tarifa no encontrada" });
      
      const checkC = await pool.query("SELECT id FROM clientes WHERE public_id = $1", [cliente_id]);
      if (checkC.rowCount === 0) return res.status(404).json({ error: "Cliente no encontrado" });

      const checkU = await pool.query("SELECT id FROM usuarios WHERE public_id = $1", [consultor_id]);
      if (checkU.rowCount === 0) return res.status(404).json({ error: "Consultor no encontrado" });
      
      const checkTA = await pool.query("SELECT id FROM tipo_asignacion WHERE public_id = $1", [tipo_asignacion_id]);
      if (checkTA.rowCount === 0) return res.status(404).json({ error: "Tipo de asignación no encontrado" });
      
      return res.status(500).json({ error: "Error desconocido al actualizar tarifa" });
    }
...
app.get("/consultorias", requireAccess({ roles: ["Administrador", "Coordinador"] }), async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      WITH c_coordinador AS (SELECT id FROM usuarios WHERE public_id = $1)
      SELECT
        c.public_id AS id,
      ...
      WHERE c.activo = true
        AND ($1::text IS NULL OR c.coordinador_responsable_id = (SELECT id FROM c_coordinador))
      ORDER BY c.id DESC
    `, [coordinador_id || null]);
    res.json(result.rows);
  } catch (err) {
    ...
    const metaConsulta = await pool.query(
      ...
      SELECT
        ...
        (SELECT estado_reporte 
         FROM reporte_horas 
         WHERE id_registro_asignacion = ra.id 
           AND estado_reporte IN ('Pendiente', 'Rechazado') 
         ORDER BY updated_at DESC NULLS LAST, id DESC LIMIT 1) AS _existente_estado
      FROM registro_asignaciones ra
        JOIN consultorias con ON ra.id_consultoria = con.id
    ...
    const cteQuery = `
      WITH
        ...
        c_valores AS (
          SELECT
            c.es_t_c_fijo,
            -- Aplicamos regla de costo total
